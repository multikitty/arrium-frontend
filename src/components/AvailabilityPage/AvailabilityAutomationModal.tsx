...
import { StyledAvailablityAutomationDeleteButton, StyledAvailablityAutomationModal, StyledAvailablityAutomationModalTitle, StyledAvailablityAutomationModalTitleDesc, StyledSearchButton } from "./AvailabilityPage.styled"
...
import { IGetPrefrencesScheduleResult, IGetPrefrencesScheduleResultData, ISetPrefrencesScheduleVariables } from "@/lib/interfaces/prefrences"
import { setPrefrencesSchedule, fetchPreferencesSchedule } from "@/agent/prefrences"
      ...
      fetchPreferencesSchedule().then(res=>{
        methods.reset({
          data: res?.data?.map((value : IGetPrefrencesScheduleResultData) => ({
            startTime: createDateInHM(Number(value.startTime.split(":")[0]), Number(value.startTime.split(":")[1])),
            active: value.active,
            day: value.day,
          }))
       })
...
>(setPrefrencesSchedule)
  ...
  return (
    <Modal open={open} onClose={handleClose} mt-5 sx={{overflow: 'scroll', alignItems: "flex-start"}} >
      <StyledAvailablityAutomationModal>
        ...
        <Box display="flex" sx={{marginBottom: "12px"}}>
          <StyledAvailablityAutomationModalTitleDesc>
            Set the days and times that you'd like to auto-start Arrium to search for blocks
          </StyledAvailablityAutomationModalTitleDesc>
    ...
    >
        ...
        >
          ...
          <ContainedButton type="submit" onClick={handleSubmit(onSubmit)}>Save</ContainedButton>
        </Box>
    </FormProvider>
