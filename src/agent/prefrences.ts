import { IGetPrefrencesResult, IGetPrefrencesScheduleResult, ISetPrefrencesResult, ISetPrefrencesScheduleVariables, ISetPrefrencesVariables } from "@/lib/interfaces/prefrences"
...
export async function fetchPreferencesSchedule(): Promise<IGetPrefrencesScheduleResult> {
    return await arriumAPI.get("/preference/schedule").then(response => response?.data )
}

export function usePreferencesSchedule(){
    return useQuery("get_preference-schedule-data", ()=> fetchPreferencesSchedule())
}
...
> = async prefrencesScheduleData => {
    return await(
        await arriumAPI.post("/preference/schedule", prefrencesScheduleData)).data
}
